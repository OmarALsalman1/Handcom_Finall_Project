...
class CarpentryForGuest extends StatefulWidget {
  const CarpentryForGuest({super.key});

  @override
  State<CarpentryForGuest> createState() => _CarpentryWorkersPageState();
}

class _CarpentryWorkersPageState extends State<CarpentryForGuest> {
  ...
  Future<void> _loadWorkers() async {
    final providers = await ProviderService.getProviders(category: 'carpentry');
    if (!mounted) return;
    setState(() {
      workers = providers
          .map((p) => Worker(
  ...
  Widget _buildCard(BuildContext context, Worker w, Color cardBg,
    ...
    final displayJob =
        '${context.l10n.carpentry} • ${context.l10n.yearsExpDisplay(w.experienceYears)}';
