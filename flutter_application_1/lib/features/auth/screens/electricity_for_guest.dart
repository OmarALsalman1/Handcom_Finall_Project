...
class ElectricityForGuest extends StatefulWidget {
  const ElectricityForGuest({super.key});

  @override
  State<ElectricityForGuest> createState() => _ElectricityWorkersPageState();
}

class _ElectricityWorkersPageState extends State<ElectricityForGuest> {
  ...
  Future<void> _loadWorkers() async {
    final providers = await ProviderService.getProviders(category: 'electrical');
    if (!mounted) return;
    setState(() {
      workers = providers
          .map((p) => Worker(
  ...
  Widget _buildCard(BuildContext context, Worker w, Color cardBg,
    ...
    final displayJob =
        '${context.l10n.electricity} • ${context.l10n.yearsExpDisplay(w.experienceYears)}';
