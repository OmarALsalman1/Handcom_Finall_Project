...
class PlumbingForGuest extends StatefulWidget {
  const PlumbingForGuest({super.key});

  @override
  State<PlumbingForGuest> createState() => _WorkersListPageState();
}

class _WorkersListPageState extends State<PlumbingForGuest> {
  ...
  Future<void> _loadWorkers() async {
    final providers = await ProviderService.getProviders(category: 'plumbing');
    if (!mounted) return;
    setState(() {
      workers = providers
          .map((p) => Worker(
  ...
  Widget _buildCard(BuildContext context, Worker w, Color cardBg,
    ...
    final displayJob =
        '${context.l10n.plumbing} • ${context.l10n.yearsExpDisplay(w.experienceYears)}';
            ...
            Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    context.l10n.statusFromRaw(w.status),
                    style: TextStyle(
                        color: statusColor,
                        fontWeight: FontWeight.bold,
                        fontSize: 11),
